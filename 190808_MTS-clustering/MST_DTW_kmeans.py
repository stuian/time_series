...
class MST_kmeans():
    ...
    def run(self):
        medoids = self.init_random_medoids()
        start = time.time()
        for iter in range(self.max_iterations):
            clusters,pred_y = self.create_clustersandindex(medoids)

            # update Center point
            for k1 in range(self.k):
                clust_ind = []
                for ind,j in enumerate(pred_y):
                    if j == k1:
                ...
                for k2 in clust_ind:
                    temp = temp + self.X[k2]
                if len(clust_ind) == 0:
                    temp = self.X[np.random.choice(range(self.X.shape[0]))]
                else:
                    temp = temp / len(clust_ind) # 0/0
                medoids[k1] = temp  # 更新
            RI = RandIndex(pred_y,self.y)
            purity = Purity(pred_y,self.y)
            nmi = NMI(pred_y,self.y)
            print("Process[%d]:RI,purity,NMI of iter %d is %.2f,%.2f,%.2f respectively" % (getpid(),iter,RI,purity,nmi))
        end = time.time()
        print(end-start)
        file = h5py.File('cricket_predict.h5', 'w')
        file.create_dataset('pred_y',data = pred_y)
